# Quickstart Guide: Chapter-Level Urdu Translation

## Prerequisites

Before setting up the translation system, ensure you have:
...
4. Verify that the "Translate to Urdu" button appears
5. Click the button and verify that translation works
...
```bash
curl -X GET http://localhost:8000/api/chapter/intro-to-robotics/translate/urdu \
  -H "Authorization: Bearer <your-token>"
```

#### Get Original Chapter Content
```bash
curl -X GET http://localhost:8000/api/chapter/intro-to-robotics \
  -H "Authorization: Bearer <your-token>"
```
...
1. **Customize Translation Prompts**: Modify the prompt templates in the backend to better suit your content
2. **Configure Translation Quality**: Adjust settings for translation quality vs speed
3. **Performance Tuning**: Adjust timeout values and caching strategies based on your requirements
4. **Monitoring**: Set up monitoring and alerting for the translation service
